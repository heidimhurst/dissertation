...
# project variables
# define project trial in initial command call (e.g. ./upsample.sh t8) to avoid default being set
trial=${1:-t9}
# factor=1.5
...
declare -a sf=(1.5 2 2.5 3 3.5 4 1)
method="lanczos"

echo "Processing upsampling for ${object_type}-type objects - trial ${trial} using method: ${method}"
...
for i in ${sf[@]}; do
	...
	echo ----------- Creating images for upsample factor $i -----------
	# create folder for upsampled images
	# mkdir ${data_path}/train_images_${method}_${factor}
	# upsample images
	python ${personal_util_path}/downsample.py \
	-i ${data_path}/train_images_${factor} \
	-o ${data_path}/train_images_upsample_${factor} \
	-r ${method} -s ${factor} -u
	...
	# copy JSON file
	echo ----------- Creating JSON file for upsample factor $i -----------
	# cp ${data_path}/xView_cars_only.geojson ${project_path}/data/${object_type}/${factor}/${prefix}_${trial}_${factor}.geojson
	python ${personal_util_path}/json_annotation_utilities.py \
	${data_path}/xView_train.geojson \
	-c -t ${id} \
	-o ${project_data_path}/${prefix}_${trial}_${factor}.geojson
	...
	${data_path}/train_images_upsample_${factor}/ \
