...
for i in ${factors[@]}; do
	...
	# path to images
	test_image_path=${data_path}/test_images_upsample_${i}
	...
	for filename in *.tif; do
		...
		# run inference
		CUDA_VISIBLE_DEVICES='1,2' python ${xview_util_path}/create_detections.py -c ${saved_model_path} -o ${output_path} ${filename}
